...
const { getUserByEmail, validateEmail, validatePhoneNumber, getUserById } = require('../models/usersModel');

const validateBody = (req, res, next) => {
    const { email, password, repassword, phoneNumber } = req.body;
    if (email && !validateEmail(email)) res.status(400).send('Not a valid email address');
    if (password && password.length < 6) res.status(400).send('Password has to be at least 6 letters');
    if (password && repassword && password !== repassword) res.status(400).send('Passwords do not match');
    if (phoneNumber && !validatePhoneNumber(phoneNumber)) res.status(400).send('Not a valid phone number');
    next();
}
